...
export default function Crossmint() {
  ...
  const disconnect = async () => {
    try {
      await _crossmintConnect.disconnect();
      setAddress("");
      setIsActive(false);
    } catch (error) {
      console.log(error);
  ...
  const createPass = async () => {
    ...
    let signature;
    try {
      signature = await _crossmintConnect.signMessage(signatureMessage);
    } catch (error) {
      console.log(error);
      return;
    } finally {
    ...
    <>
        ...
        <main>
          <div className="flex items-center justify-center my-8">
            <button
              ...
              onClick={!address ? login : () => setIsActive(true)}
            >
              {address
      ...
      <Modal isActive={isActive} onClose={() => setIsActive(false)}>
        <div className="flex flex-col items-center justify-center gap-4">
          ...
          <button
            className="w-full rounded-lg bg-red-600 text-white transition duration-300 ease-in-out hover:bg-indigo-500 px-4 py-2"
            onClick={disconnect}
          >
            Sign Out
          </button>
